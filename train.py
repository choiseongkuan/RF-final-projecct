...
# torch.backends.cudnn.benchmark = False
# # 算法初始化
# PPO_policy = PPO.PPO(dim_info[0], dim_info[1], args["Adam_stepsize"], args["Horizon"], tricks)
# # 训练
# step = 0
# episode_num = 0
# episode_reward = 0
# state, info = env.reset(seed=args["seed"])
# if tricks['state_normalization']:  # 通用的提升训练效果方法
#     state_norm = Normalization(shape=dim_info[0])
#     state = state_norm(state)
# if tricks['reward_normalization']:  # 限制rewards的尺度，避免太多或太小
...
#     next_state, reward, terminated, truncated, infos = env.step(action_)
#
#     if tricks['state_normalization']:
#         next_state = state_norm(next_state)
#     if tricks['reward_normalization']:
#         reward_ = reward_norm(reward)
#     done = terminated or truncated
#     done_bool = terminated  ### truncated 为超过最大步数
#     PPO_policy.add(state, action, reward, next_state, done_bool, action_log_pi, done)
#     if tricks['reward_normalization']:
#         PPO_policy.add(state, action, reward_, next_state, done_bool, action_log_pi, done)
#     else:
#         PPO_policy.add(state, action, reward, next_state, done_bool, action_log_pi, done)
#     episode_reward += reward
#     state = next_state
...
#     if step % args["Horizon"] == 0:
#         print(step)
#         PPO_policy.update(args["Minibatch_size"], args["Discount"], args["GAE_parameter"], args["clip_parameter"],
#                       args["Num_epochs"],
